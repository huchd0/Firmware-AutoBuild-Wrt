...
if [ -n "\$ROOT_DEV" ]; then
    ...
    if ! lsblk | grep -q "\${PART_DEV##*/}"; then
        ...
        blockdev --rereadpt "\$ROOT_DEV" >/dev/null 2>&1 || true
        sleep 3
    fi

    # 只读取现有分区的 UUID 不执行格式化
    if lsblk | grep -q "\${PART_DEV##*/}"; then
        TARGET_UUID=\$(blkid -s UUID -o value "\$PART_DEV" 2>/dev/null)
        
        # 只有读取到合法的 UUID 才会执行挂载操作
...
if [ -n "$openclash_before" ] && [ "$openclash_before" != "$openclash_after" ]; then
    echo "OpenClash 已升级正在重启服务" >> "$LOGFILE"
...
declare -a PKG_LIST=(
    # 核心网络控制
    "-dnsmasq"                          # 卸载自带 dnsmasq
    "dnsmasq-full"                      # 安装完整 dnsmasq-full
    ...
    "kmod-fs-ext4"                      # ext4 驱动
    "kmod-fs-ntfs3"                     # ntfs 驱动
    "kmod-fs-vfat"                      # vfat 驱动
    ...
    "curl"                              # 网络请求
    "ca-bundle"                         # 根证书包
    "ca-certificates"                   # 完整根证书库
    "libustream-openssl"                # HTTPS 下载支持
    "-wget"                             # 卸载简配版 wget
    "wget-ssl"                          # 完整版 wget
    "libcap"                            # 权限控制库
    "libcap-bin"                        # 权限控制工具
    ...
    "kmod-igc"                          # Intel 2.5G 驱动
    "kmod-igb"                          # Intel 千兆驱动
    "kmod-r8169"                        # Realtek 驱动
    "iwinfo"                            # 无线硬件信息工具
    "kmod-tcp-bbr"                      # BBR 拥塞控制
    "kmod-nft-offload"                  # 流量卸载引擎

    # 无线与蓝牙底层驱动
    "-wpad-basic-mbedtls"               # 卸载简配版 WPA 认证
    "-wpad-basic-wolfssl"               # 卸载简配版 WPA 认证
    ...
    "iftop"                             # 流量监控
    "screen"                            # 多窗口终端
    "collectd-mod-thermal"              # 温度监控
    "collectd-mod-sensors"              # 传感器数据
    "collectd-mod-cpu"                  # CPU 负载
    "collectd-mod-ping"                 # 网络延迟
    "collectd-mod-interface"            # 接口流量
    "collectd-mod-rrdtool"              # 数据库落盘
    "collectd-mod-iwinfo"               # 无线状态

    # 扩展应用插件
    "luci-app-ttyd"                     # 网页终端命令行
    "luci-i18n-ttyd-zh-cn"              # 网页终端中文包
    "luci-app-samba4"                   # Samba4 共享
    "luci-i18n-samba4-zh-cn"            # Samba4 中文包
    "wsdd2"                             # Samba 发现服务
    "luci-app-nlbwmon"                  # 网络带宽监控
    "luci-i18n-nlbwmon-zh-cn"           # 带宽监控中文包
    "luci-app-statistics"               # 性能统计图表
    "luci-i18n-statistics-zh-cn"        # 性能图表中文包
    "luci-app-upnp"                     # UPnP 端口转发
    "luci-i18n-upnp-zh-cn"              # UPnP 中文包
    "miniupnpd-nftables"                # UPnP 底层服务
)
