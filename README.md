...
> **💡 提示**：在 OpenWrt/ImmortalWrt 编译时，**架构 (Arch)** 选错会导致固件无法运行（变砖）或无法刷入。请务必先核对芯片型号对应的架构(arch)和硬件设备专属的 Profile 名称。（比如：OpenWrt/ImmortalWrt 社区的一个“命名历史遗留问题，匹配品牌xiaomi时会出现redmi，由于红米 AX6000 的全名是 xiaomi_redmi-router-ax6000，它完美包含了 xiaomi，所以被引擎“由于太匹配”而抓取了出来，但只要填写正确的架构(arch)和硬件设备专属的 Profile 名称那生成的固件也会是正确的）
