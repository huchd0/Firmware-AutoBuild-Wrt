...
嵌入式路由器中芯片决定属于哪个架构(arch)，每一个具体的硬件型号都有自己专属的 Profile 名称（查询方式-->>>禁用build_embedded.sh最后的执行构建后，临时使本页代码最后一行make info生效运行，在过程的第四步Run Builder，中间有一行Available Profiles:下面，有很多路由器信息块，第一行冒號左側的字符串（例如 xiaomi_mi-router-4g）就是界面中要填入 device_profile 輸入框的值）。
